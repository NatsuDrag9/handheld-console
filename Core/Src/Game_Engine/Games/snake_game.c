...
//    SnakeGameData* data = (SnakeGameData*)snake_game_engine.game_data;
//
//    // On first render, draw everything
//    if (first_render) {
//
//        // Draw score and lives initially
//        char status_text[32];
...
//    }
//
//    // Clear previous head position if it moved
//    if (previous_head_x != data->head_x || previous_head_y != data->head_y) {
//        // Only clear the previous head position if it's not currently occupied by a body segment
...
//
//        if (!is_occupied) {
//            // Check if we're near borders before clearing
...
//            } else {
//                // We're near border - clear carefully and redraw border if needed
//                display_clear_region(previous_head_x, previous_head_y, SPRITE_SIZE, SPRITE_SIZE);
//                display_draw_border_at(1, STATUS_START_Y, 3, 3); // Redraw border
//            }
//        }
//    }
//
//    // Clear previous snake tail position if it's no longer part of the snake
//    // Only needed when snake didn't grow
//    if (previous_length == data->length &&
//        (previous_tail_x != 0 || previous_tail_y != 0)) {
//        // Check if the previous tail position is not part of the current snake
...
//                );
//            } else {
//                // Near border - just clear the exact area
//                display_clear_region(previous_tail_x, previous_tail_y, SPRITE_SIZE, SPRITE_SIZE);
//                display_draw_border_at(1, STATUS_START_Y, 3, 3); // Redraw border
//            }
//        }
//    }
//
//    // Clear previous food position if food moved
...
//
//        // If food was near border, redraw the border
//        if (previous_food_x <= BORDER_OFFSET + SPRITE_SIZE ||
//            previous_food_x >= DISPLAY_WIDTH - BORDER_OFFSET - SPRITE_SIZE - 1 ||
//            previous_food_y <= GAME_AREA_TOP + SPRITE_SIZE ||
//            previous_food_y >= DISPLAY_HEIGHT - BORDER_OFFSET - SPRITE_SIZE - 1) {
//            display_draw_border_at(1, STATUS_START_Y, 3, 3);
//        }
//    }
//
